import requests, json, datetime, base64, asyncio, os
...
def incremental():
    ...
    print(f"Existing Meetings: {len(existing_classes)}")
    print(f"Featured Meetings: {len(featured_classes)}")
    print(f"New Meetings: {len(new_classes)}")

    async def submit_existing_class(data):
            #print(data)

        print(f"Submitting existing Meeting: {data['cobalt_meetingId']} - {data['cobalt_Location']} - {data['cobalt_name']} - {data['cobalt_price']} - {data['cobalt_cobalt_tag_cobalt_meeting']}")
        ...
        if data['cobalt_Location'] != []:
            ramco_class["venue"] = data['cobalt_Location']

        #payload = urlencode(ramco_class)
        #print(payload)
        url = f"{config['WORDPRESS_URL']}/events/by-slug/{data['cobalt_meetingId']}"
        headers = {
    ...
    async def submit_featured_class(data):
        print(f"Submitting featured Meeting: {data['cobalt_meetingId']} - {data['cobalt_Location']}")
        ramcoClass = {
            "title": data['cobalt_name'],
            "status": "publish",
            "hide_from_listings": data['publish'],
