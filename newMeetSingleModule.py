...
def newMeetSingle(guid, staging):
    ...
    def process_classes(obj):
        ...
        #format start date
        startDate = datetime.fromtimestamp(obj['cobalt_BeginDate']['Value'])
        obj['cobalt_BeginDate']['Display'] = startDate.strftime("%Y-%m-%d %H:%M:%S")

        #format end date
        endDate = datetime.fromtimestamp(obj['cobalt_EndDate']['Value'])
        ...
        if len(obj['cobalt_cobalt_tag_cobalt_meeting']) > 0:
            for item in obj['cobalt_cobalt_tag_cobalt_meeting']:
                ...
                if len(resultTag)>0:
                    ...
                    print(f"Tag not found in wordpress for ***{obj['cobalt_name']}*** with id ***{obj['cobalt_meetingId']}*** : {item['cobalt_name']}")
                resultCat = [cat['id'] for cat in catSearch if cat['name'] == item['cobalt_name']]
                if len(resultCat)>0:
                    categories.append(resultCat[0])
                else:
                    print(f"Category not found in wordpress for ***{obj['cobalt_name']}*** with id ***{obj['cobalt_meetingId']}*** : {item['cobalt_name']}")
        else:
            tags.append(1660)
        ...
        if len(resultVenue) > 0:
            ...
            print(f"Venue not found in wordpress for ***{obj['cobalt_name']}*** with id ***{obj['cobalt_meetingId']}*** : {cobalt_location_id}")
            obj['cobalt_Location'] = []
        ...
        print(f"Class processed: {obj['cobalt_name']} - {obj['cobalt_meetingId']} - {obj['cobalt_Location']} - {obj['cobalt_price']} - {obj['cobalt_cobalt_tag_cobalt_meeting']}")
    ...
    def check_if_exists(obj):
        ...
        if response.status_code == 200:
            ...
            if response["image"] == False:
                #obj['cobalt_cobalt_tag_cobalt_meeting'] = filtered_tags
    ...
    def submit_new_class(data):
        print(f"Submitting new class: {data['cobalt_name']} - {data['cobalt_meetingId']}")
        ramcoClass = {
